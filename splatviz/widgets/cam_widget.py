...
class CamWidget(Widget):
    def __init__(self, viz):
        ...
        self.control_modes = ["Orbit", "WASD", "COLMAP"]
    ...
    def __call__(self, show: bool):
        ...
        self.handle_wasd()
        self.handle_colmap_control()
        if show:
            label("Control Mode", viz.label_w)
            _, self.current_control_mode = imgui.combo("##cam_modes", self.current_control_mode, self.control_modes)
            if self.control_modes[self.current_control_mode] == "COLMAP":
                label("Camera Index", viz.label_w)
                _, self.camera_idx = imgui.input_int("##camera_idx", self.camera_idx)
            else:
    ...
    def handle_colmap_control(self):
        if self.control_modes[self.current_control_mode] == "COLMAP":
            assert self.viz.cameras is not None, "Cameras not found"
            
            cdict = self.viz.cameras[self.camera_idx]
            self.viz.args.camera_idx = self.camera_idx
            self.viz.args.current_camera = cdict
            R = torch.tensor([
                cdict['rotation'][0],
                cdict['rotation'][1],
                cdict['rotation'][2],
            ])
            t = torch.tensor(cdict['position'])
            
            extr = torch.eye(4)
            extr[:3, :3] = R
            extr[:3, 3] = t
            
            self.viz.args.cam_params = extr
            
            from utils.graphics_utils import focal2fov
            import math
            self.viz.args.fov = math.degrees(focal2fov(cdict['fx'], cdict['width']))
